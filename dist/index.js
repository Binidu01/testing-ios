...
${a.bold}${a.cyan}Examples:${a.reset}
  ...
  ${a.dim}create-bini-app my-app --platform android --nosign --pnpm${a.reset}
    `),w(0));let i=e.includes("--typescript"),n=e.includes("--javascript")||e.includes("--no-typescript"),r=e.includes("--tailwind"),o=e.includes("--css-modules"),s=e.includes("--none"),c=e.includes("--install"),d=e.includes("--no-install"),f=e.indexOf("--platform"),g=f!==-1?e[f+1]:void 0,b=e.indexOf("--app-name"),l=b!==-1?e[b+1]:void 0,u=e.includes("--sign"),X=e.includes("--nosign"),k={"--npm":"npm","--pnpm":"pnpm","--yarn":"yarn","--bun":"bun"},I=Object.keys(k).filter(C=>e.includes(C));i&&n&&(t.error("Cannot use --typescript and --javascript together."),w(1)),[r,o,s].filter(Boolean).length>1&&(t.error("Cannot use more than one of --tailwind, --css-modules, --none."),w(1)),c&&d&&(t.error("Cannot use --install and --no-install together."),w(1)),u&&X&&(t.error("Cannot use --sign and --nosign together."),w(1)),I.length>1&&(t.error(`Cannot use more than one of ${Object.keys(k).join(", ")} together.`),w(1));let J=I.length===1?k[I[0]]:void 0;return {projectName:e.find(C=>!C.startsWith("--")&&C!==g&&C!==l),flags:{force:e.includes("--force"),typescript:i?true:n?false:void 0,javascript:n,tailwind:r,cssModules:o,noStyle:s,install:c?true:d?false:void 0,platform:g,appName:l,sign:u?true:X?false:void 0,packageManager:J}}}function W(e){return !e||e.length>50||te.test(e)?false:!ne.test(e)}function se(e){return ["web","windows","linux","macos","android","ios"].includes(e)}function q(){let e=process.platform;return e==="win32"?"windows":e==="darwin"?"macos":e==="linux"?"linux":"windows"}function U(e){try{h.mkdirSync(e,{recursive:!0,mode:488});}catch(i){throw new Error(`Cannot create directory: ${e}. ${i.message}`)}}async function v(e,i,n={}){await x.mkdir(p.dirname(e),{recursive:true,mode:488}),await x.writeFile(e,i,{mode:n.mode??416,flag:n.flag??"w"});}function le(e,i={}){let n=i.allowedBase??process.cwd();if(!e)throw new Error("Path required.");let r=p.resolve(e),o=p.resolve(n),s=p.relative(o,r);if(s===""||s.startsWith("..")||p.isAbsolute(s))throw new Error(`Refusing to delete: "${r}" is outside the allowed base "${o}".`);if([p.resolve("/"),process.env.HOME?p.resolve(process.env.HOME):null,process.env.USERPROFILE?p.resolve(process.env.USERPROFILE):null].filter(f=>f!==null).includes(r))throw new Error("Refusing to delete a system directory.");if(r.split(p.sep).filter(Boolean).length<2)throw new Error("Refusing to delete a root-level directory (safety check).");h.existsSync(r)&&h.rmSync(r,{recursive:true,force:true});}function O(e,i={}){try{let n=execSync(e,{shell:process.platform==="win32"?"cmd.exe":"/bin/sh",stdio:i.stdio??"pipe",timeout:i.timeout??12e4,cwd:i.cwd,windowsHide:!0,encoding:"utf8"});return String(n)}catch(n){throw new Error(`Command failed: ${e}
${n.message??String(n)}`)}}function T(e){try{let i=process.platform==="win32"?`where ${e}`:`which ${e}`;return O(i,{stdio:"ignore"}),!0}catch{return  false}}function j(e,i){try{execSync(e,{cwd:i,stdio:"ignore",shell:process.platform==="win32"?"cmd.exe":"/bin/sh",timeout:12e4,windowsHide:!0});}catch{}}function D(e,i){switch(e){case "npm":return `npx ${i}`;case "yarn":return `yarn ${i}`;case "pnpm":return `pnpm ${i}`;case "bun":return `bunx ${i}`}}function m(e,i){return e==="npm"?`npm run ${i}`:`${e} ${i}`}var E={bun:"bun --version",pnpm:"pnpm --version",yarn:"yarn --version",npm:"npm --version"};function de(){let i=[{name:"bun",command:E.bun,priority:4},{name:"pnpm",command:E.pnpm,priority:3},{name:"yarn",command:E.yarn,priority:2},{name:"npm",command:E.npm,priority:1}].filter(n=>{try{return O(n.command,{stdio:"ignore"}),!0}catch{return  false}});if(i.length===0)throw new Error("No package manager found. Install npm, yarn, pnpm, or bun.");return i.sort((n,r)=>r.priority-n.priority)[0].name}function ce(e){if(e)try{return O(E[e],{stdio:"ignore"}),{pm:e,failed:!1,forced:!0}}catch{t.error(`Requested package manager "${e}" was not found on PATH.`),w(1);}try{return {pm:de(),failed:!1,forced:!1}}catch(i){return t.warn(`Could not detect package manager: ${i.message}`),{pm:"npm",failed:true,forced:false}}}async function pe(e,i,n){if(!n)return  false;let r={npm:"npm install --no-audit --no-fund --loglevel=error",yarn:"yarn install --silent --no-progress",pnpm:"pnpm install --reporter=silent",bun:"bun install --silent"};t.step(`Installing dependencies with ${i}...`);try{return O(r[i],{cwd:e,stdio:"inherit",timeout:3e5}),t.success("Dependencies installed."),!0}catch{return t.warn("Auto-install failed. Run manually:"),t.plain(`    ${a.green}cd ${p.basename(e)}${a.reset}`),t.plain(`    ${a.green}${i} install${a.reset}`),false}}function ue(e,i){return e.typescript===true?true:e.javascript===true?false:i.typescript}function me(e){let i=e?"ts":"js";return {main:e?"tsx":"jsx",config:i,api:i}}async function ge(e,i){let n;e.typescript!==void 0?n=e.typescript:A()?n=await $({message:"Use TypeScript?",default:true}):n=true;let r;e.tailwind?r="Tailwind":e.cssModules?r="CSS Modules":e.noStyle?r="None":A()?r=await select({message:"Styling solution?",choices:[{name:"Tailwind CSS",value:"Tailwind"},{name:"CSS Modules",value:"CSS Modules"},{name:"None",value:"None"}],default:"Tailwind"}):r="Tailwind";let o,s=q();if(e.platform&&se(e.platform))o=e.platform;else if(A()){let d=[{name:"Web Application",value:"web"},{name:"Windows Desktop",value:"windows"},{name:"Linux Desktop",value:"linux"},{name:"macOS Desktop",value:"macos"},{name:"Android",value:"android"},{name:"iOS",value:"ios"}];console.log(`
Detected OS: ${s}`),o=await select({message:"Select target platform:",choices:d,default:"web"});}else o="web";let c=i;return o!=="web"&&(e.appName?c=e.appName:A()?c=await input({message:"App name? (used as the app name and window title)",default:L(i),validate:d=>d.trim().length>0?true:"Required."}):c=L(i)),{typescript:n,styling:r,platform:o,appName:c}}async function fe(e){let i=["favicon.ico","apple-touch-icon.png","og-image.png","logo.png"];await Promise.all(i.map(async n=>{let r=p.join(ee,n),o=p.join(e,n);try{await x.access(r),await x.copyFile(r,o);}catch{t.warn(`Asset not found, skipping: ${n}`);}}));}async function be(e){let i={name:"Bini.js App",short_name:"BiniApp",description:"Modern React application built with Bini.js",start_url:"/",display:"standalone",background_color:"#ffffff",theme_color:"#00CFFF",icons:[{src:"/favicon.ico",sizes:"64x64 32x32 24x24 16x16",type:"image/x-icon"},{src:"/apple-touch-icon.png",sizes:"180x180",type:"image/png"}]};await v(p.join(e,"public","site.webmanifest"),JSON.stringify(i,null,2));}async function he(e,i,n,r,o){let s=r==="Tailwind"?`import tailwindcss from '@tailwindcss/vite';
`:"",c=r==="Tailwind"?`
      tailwindcss(),`:"",d=i?`,
    types: ["vite/client"]`:"",f=o?`import { biniNative } from 'bini-native';
`:"",g=o?`
      biniNative(),`:"",b=["'**/dist/**'","'**/node_modules/**'"];o&&(b.push("'**/src-tauri/**'"),b.push("'**/target/**'"),b.push("'**/*.exe'"),b.push("'**/*.dll'"),b.push("'**/*.pdb'"));let l=b.join(`,
...
certificate store (Import-PfxCertificate). See the docs if you haven't done that yet.${a.reset}`);let r=await input({message:"Certificate thumbprint (Personal > Certificates > Details in certmgr.msc):",validate:d=>d.trim().length>0?true:"Required."}),o=await select({message:"Digest algorithm:",choices:[{name:"sha256",value:"sha256"},{name:"sha1",value:"sha1"}],default:"sha256"}),s=await input({message:"Timestamp server URL:",default:"http://timestamp.comodoca.com"});await K(e,d=>{d.bundle=d.bundle??{},d.bundle.windows={...d.bundle.windows??{},certificateThumbprint:r,digestAlgorithm:o,timestampUrl:s};})&&(t.success("Windows signing configured in tauri.conf.json"),t.info("Cross-compiling from Linux/macOS requires a custom signCommand instead \u2014 see the docs."));}async function ke(e,i,n){if(t.step("macOS code signing"),!(n===true?true:await $({message:"Configure macOS code signing now?",default:false}))){t.info("Skipped. See: https://v2.tauri.app/distribute/sign/macos/");return}let o=await select({message:"Signing method:",choices:[{name:"Ad-hoc (local testing, no Apple Developer account)",value:"adhoc"},{name:"Apple Developer signing identity (Distribution / Developer ID)",value:"identity"}],default:"adhoc"}),s=o==="adhoc"?"-":await input({message:'Signing identity (from "security find-identity -v -p codesigning"):',validate:l=>l.trim().length>0?true:"Required."});if(await K(i,l=>{l.bundle=l.bundle??{},l.bundle.macOS={...l.bundle.macOS??{},signingIdentity:s};})&&t.success(`macOS signingIdentity set to "${s}" in tauri.conf.json`),o!=="identity"||!await $({message:"Set up notarization credentials too? (avoids the 'unidentified developer' warning)",default:false}))return;let f=await select({message:"Notarization method:",choices:[{name:"App Store Connect API key",value:"apiKey"},{name:"Apple ID + app-specific password",value:"appleId"}],default:"apiKey"}),g={};f==="apiKey"?(g.APPLE_API_ISSUER=await input({message:"APPLE_API_ISSUER (Issuer ID):"}),g.APPLE_API_KEY=await input({message:"APPLE_API_KEY (Key ID):"}),g.APPLE_API_KEY_PATH=await input({message:"Path to downloaded .p8 private key:",validate:l=>h.existsSync(l)?true:"File not found."})):(g.APPLE_ID=await input({message:"Apple ID email:"}),g.APPLE_PASSWORD=await password({message:"App-specific password:",mask:"*"}),g.APPLE_TEAM_ID=await input({message:"Apple Team ID:"}));let b=p.join(e,".env.signing");await F(b,g,"macOS notarization credentials \u2014 never commit this file.\nRun `source .env.signing` before `pnpm tauri:build`."),await R(e,["# Code signing secrets (never commit)",".env.signing"]),t.success(`Wrote notarization credentials to ${p.relative(e,b)}`),t.info('Run "source .env.signing" before building to notarize your app.');}async function Se(e,i){if(t.step("Linux AppImage signing"),!(i===true?true:await $({message:"Configure AppImage signing (gpg) now?",default:false}))){t.info("Skipped. See: https://v2.tauri.app/distribute/sign/linux/");return}if(!T("gpg")&&!T("gpg2")){t.warn("gpg/gpg2 not found. Install it, generate a key with `gpg2 --full-gen-key`, then re-run.");return}let r=await input({message:"GPG key ID to sign with (blank = default key):"}),s={SIGN:"1",APPIMAGETOOL_SIGN_PASSPHRASE:await password({message:"GPG key passphrase:",mask:"*"})};r.trim()&&(s.SIGN_KEY=r.trim());let c=p.join(e,".env.signing");await F(c,s,"AppImage signing secrets \u2014 never commit this file.\nRun `source .env.signing` before `pnpm tauri:build`."),await R(e,["# Code signing secrets (never commit)",".env.signing"]),t.success(`Wrote AppImage signing config to ${p.relative(e,c)}`),t.info('Run "source .env.signing" before building to sign the AppImage.');}async function Ae(e,i){if(t.step("iOS code signing"),!(i===true?true:await $({message:"Configure iOS code signing now?",default:false}))){t.info("Skipped. Xcode-managed automatic signing is used by default.");return}if(await select({message:"Signing method:",choices:[{name:"Automatic (Xcode-managed, recommended for local builds)",value:"automatic"},{name:"Manual (certificate + provisioning profile, for CI)",value:"manual"}],default:"automatic"})==="automatic"){t.info("Nothing to configure locally \u2014 sign in with your Apple ID in Xcode (Settings > Accounts).");return}let o=await input({message:"Path to exported certificate (.p12):",validate:d=>h.existsSync(d)?true:"File not found."}),s=await password({message:"Certificate export password:",mask:"*"}),c=await input({message:"Path to provisioning profile (.mobileprovision):",validate:d=>h.existsSync(d)?true:"File not found."});try{let d=(await x.readFile(o)).toString("base64"),f=(await x.readFile(c)).toString("base64"),g=p.join(e,".env.signing");await F(g,{IOS_CERTIFICATE:d,IOS_CERTIFICATE_PASSWORD:s,IOS_MOBILE_PROVISION:f},"iOS manual signing secrets \u2014 never commit this file.\nRun `source .env.signing` before `pnpm tauri ios build`."),await R(e,["# Code signing secrets (never commit)",".env.signing"]),t.success(`Wrote iOS signing credentials to ${p.relative(e,g)}`),t.info('Run "source .env.signing" before building to sign your iOS app.');}catch(d){t.warn(`Could not read/encode certificate or profile: ${d.message}`);}}async function Pe(e,i,n,r){if(r===false){t.info("Skipping code signing setup (--nosign). See https://v2.tauri.app/distribute/sign/");return}if(!A()){t.info("Skipping code signing setup (non-interactive). See https://v2.tauri.app/distribute/sign/");return}switch(n){case "android":await Xe(e,i,r);break;case "windows":await $e(i,r);break;case "macos":await ke(e,i,r);break;case "linux":await Se(e,r);break;case "ios":await Ae(e,r);break}}async function Ce(e,i,n,r,o,s,c){t.step(`Setting up Tauri for ${i} on ${n}`),t.step("Installing Tauri dependencies...");let d=[{type:"dev",packages:["@tauri-apps/cli@latest","cross-env@latest","bini-native@latest"]},{type:"prod",packages:["@tauri-apps/api@latest"]}];for(let l of d){let u=l.type==="dev"?`${r} add -D ${l.packages.join(" ")}`:`${r} add ${l.packages.join(" ")}`;try{j(u,e);}catch{}}t.success("Tauri dependencies installed");let f=p.join(e,"src-tauri");if(h.existsSync(f))t.skip("Tauri already initialized");else {t.step("Initializing Tauri with auto-filled values...");let l=`npx @tauri-apps/cli init       --app-name "${s}"       --window-title "${s}"       --frontend-dist "../dist"       --dev-url "http://localhost:3000"       --before-dev-command "${m(r,"dev")}"       --before-build-command "${m(r,"build")}"       --force`;t.command(l);try{execSync(l,{cwd:e,stdio:"inherit",timeout:6e4,shell:process.platform==="win32"?"cmd.exe":"/bin/sh",env:{...process.env,FORCE_COLOR:"true"}}),t.success("Tauri initialized with auto-filled values");}catch{t.warn("Tauri init failed. Please run manually:"),t.plain(`  ${a.yellow}${l}${a.reset}`);return}}await ve(f,o);let g=p.join(f,"icons");if(h.existsSync(g)){t.step("Removing existing Tauri icons...");try{h.rmSync(g,{recursive:!0,force:!0}),t.success("Existing icons removed");}catch(l){t.warn(`Could not remove icons: ${l instanceof Error?l.message:String(l)}`);}}t.step("Generating Tauri icons from public/logo.png...");let b=D(r,"tauri icon public/logo.png");t.command(b);try{execSync(b,{cwd:e,stdio:"inherit",timeout:6e4,shell:process.platform==="win32"?"cmd.exe":"/bin/sh"}),t.success("Tauri icons generated from logo.png");}catch{t.warn("Could not generate icons automatically."),t.info(`Run: ${a.cyan}${b}${a.reset}`);}if(i==="android"){let l=p.join(e,"src-tauri","gen","android");if(h.existsSync(l))t.skip("Android support already initialized");else {t.step("Initializing Android support...");let u=D(r,"tauri android init");t.command(u);try{execSync(u,{cwd:e,stdio:"inherit",timeout:12e4,shell:process.platform==="win32"?"cmd.exe":"/bin/sh"}),t.success("Android support initialized");}catch{t.warn("Android init failed. Please run manually:"),t.plain(`  ${a.yellow}${u}${a.reset}`);}}}if(i==="ios"){let l=p.join(e,"src-tauri","gen","ios");if(h.existsSync(l))t.skip("iOS support already initialized");else {t.step("Initializing iOS support...");let u=D(r,"tauri ios init");if(t.command(u),n==="macos")try{execSync(u,{cwd:e,stdio:"inherit",timeout:12e4,shell:process.platform==="win32"?"cmd.exe":"/bin/sh"}),t.success("iOS support initialized");}catch{t.warn(`iOS init failed. Please run manually: ${u}`);}else t.warn("iOS initialization skipped (requires macOS)");}}if(await Pe(e,f,i,c),t.step("Setup Instructions"),t.plain(we(i,n)),t.step("Checking prerequisites..."),i==="android"){T("java")||t.warn("Java JDK 17 not found (required for Android)"),process.env.ANDROID_HOME||t.warn("ANDROID_HOME not set (required for Android)"),t.step("Adding Rust Android targets");let l=["aarch64-linux-android","armv7-linux-androideabi","i686-linux-android","x86_64-linux-android"];for(let u of l){t.command(`rustup target add ${u}`);try{execSync(`rustup target list | grep ${u}`,{stdio:"pipe",shell:process.platform==="win32"?"cmd.exe":"/bin/sh"}).toString().includes("installed")||j(`rustup target add ${u}`,e);}catch{j(`rustup target add ${u}`,e);}}t.success("Rust Android targets ready");}if(i==="ios"&&n!=="macos"&&t.warn("iOS development requires macOS with Xcode"),n==="windows"&&i==="windows"&&!T("cl")&&t.warn("Visual Studio Build Tools not found (required for Windows)"),t.success(`Tauri setup complete for ${i}`),t.step("Available Commands"),i==="android"){let l=m(r,"android"),u=m(r,"android:build");t.plain(`
  ${a.green}${a.bold}Run on Android:${a.reset} ${a.cyan}${l}${a.reset}`),t.plain(`  ${a.green}${a.bold}Build APK:${a.reset} ${a.cyan}${u}${a.reset}`),t.plain(`  ${a.green}${a.bold}Manual Command:${a.reset} ${a.dim}npx @tauri-apps/cli android dev${a.reset}
`),t.plain(`  ${a.yellow}${a.bold}Quick Start Guide:${a.reset}`),t.plain("  1. Start an Android emulator or connect a device with USB debugging"),t.plain(`  2. Run: ${a.green}${l}${a.reset}`),t.plain(`  3. Build APK: ${a.green}${u}${a.reset}
...
${i}`:e==="CSS Modules"?i:i+`
`+Ne()}function Re(e,i,n,r){let o=r!=="web",s={react:"latest","react-dom":"latest","react-router-dom":"latest",hono:"latest","bini-router":"latest","bini-overlay":"latest","bini-server":"latest"},c={"@vitejs/plugin-react":"latest",vite:"latest",oxlint:"latest",oxfmt:"latest","bini-env":"latest","bini-export":"latest"};o&&(c["@tauri-apps/cli"]="latest",c["cross-env"]="latest",c["bini-native"]="latest",s["@tauri-apps/api"]="latest"),i&&Object.assign(c,{"@types/react":"latest","@types/react-dom":"latest","@types/node":"latest",typescript:"latest"}),n==="Tailwind"&&Object.assign(c,{tailwindcss:"latest","@tailwindcss/vite":"latest"});let d={dev:o?"vite":"vite --host --open",build:i?"tsc --noEmit && vite build":"vite build",export:"vite build --mode export",start:"bini-server",preview:"vite preview --host --open","type-check":i?"tsc --noEmit":"echo 'TypeScript not enabled'",lint:"oxlint src",format:"oxfmt src",check:i?"oxlint src && oxfmt src && tsc --noEmit":"oxlint src && oxfmt src",android:"npx @tauri-apps/cli android dev","android:dev":"npx @tauri-apps/cli android dev","android:build":"npx @tauri-apps/cli android build",ios:"npx @tauri-apps/cli ios dev","ios:dev":"npx @tauri-apps/cli ios dev","ios:build":"npx @tauri-apps/cli ios build"};return o&&(d.predev="npx @tauri-apps/cli icon public/logo.png",d.prebuild="npx @tauri-apps/cli icon public/logo.png",d["tauri:dev"]="cross-env TAURI=true tauri dev",d["tauri:build"]="cross-env TAURI=true tauri build",d["tauri:icon"]="npx @tauri-apps/cli icon public/logo.png"),{name:e,type:"module",version:"1.0.0",scripts:d,dependencies:s,devDependencies:c}}var B=`[
...
`}async function je(e,i,n,r,o){let s=p.join(e,"src/app"),c=n?"{ children }: { children: React.ReactNode }":"{ children }",d=`import React from 'react';
...
`;await Promise.all([v(p.join(e,"src",`main.${i}`),d),v(p.join(s,`layout.${i}`),`import React from 'react';
import './globals.css';
...
`),v(p.join(s,`page.${i}`),Ie(i,r))]);}async function De(e,i){await v(p.join(e,`src/app/api/hello.${i}`),`import { Hono } from 'hono'
...
| \`npx @tauri-apps/cli ios build\` | Manual equivalent of \`${o}\` |`,quickStart:["Make sure Xcode is installed and you're signed in with an Apple ID (Xcode > Settings > Accounts).",`Run \`${n}\` to launch the Simulator build.`,`Run \`${o}\` to produce a build for TestFlight / App Store submission.`]}}case "windows":case "linux":case "macos":{let n=e==="windows"?"Windows":e==="macos"?"macOS":"Linux",r=m(i,"tauri:dev"),o=m(i,"tauri:build"),s=m(i,"tauri:icon");return {features:[`Ships a native ${n} desktop binary \u2014 the frontend runs inside the OS's system webview (WebView2 on Windows, WebKitGTK on Linux, WKWebView on macOS), so bundle sizes stay small.`,"Full access to the filesystem, clipboard, notifications, and dialogs via `@tauri-apps/api`, auto-wired by `bini-native` as you use them.","External links open in the user's default browser instead of the app window.",`Code signing (${e==="windows"?"Authenticode certificate":e==="macos"?"Developer ID + notarization":"GPG-signed AppImage"}) can be configured during scaffolding or later \u2014 see the Code Signing note below.`],commandsTable:`| Command | Description |
|---------|-------------|
| \`${r}\` | Start the app in development mode |
| \`${o}\` | Build a distributable ${n} binary |
| \`${s}\` | Regenerate app icons from \`public/logo.png\` |`,quickStart:[`Run \`${r}\` to launch the desktop app.`,`Run \`${o}\` when you're ready to produce a distributable build.`]}}}}function Me(e,i,n,r,o,s){let c="";if(o&&s){let{features:f,commandsTable:g,quickStart:b}=Be(s,r),l=s==="windows"?"Windows":s==="macos"?"macOS":s==="linux"?"Linux":s==="android"?"Android":"iOS";c=`
## ${l} Desktop / Mobile Support

This project includes Tauri, targeting **${l}**.
...
${g}
`;}let d=o?s==="android"?m(r,"android"):s==="ios"?m(r,"ios"):m(r,"tauri:dev"):m(r,"dev");return `# ${e}
...
`}async function Le(e,i,n,r){let o=p.join(process.cwd(),e),s=p.join(o,"public");h.existsSync(o)&&!n.force&&(t.error(`Directory "${e}" already exists. Use --force to overwrite.`),w(1)),n.force&&h.existsSync(o)&&(t.warn(`Removing existing directory: ${a.dim}${o}${a.reset}`),le(o)),t.info(`Creating project in ${a.cyan}${o}${a.reset}`),re(100);let c=ue(n,i),d=me(c);U(p.join(o,"src/app/api")),U(s),t.info("Scaffolding project files");let f=i.platform!=="web",g=f?i.platform:null,b=Re(e,c,i.styling,i.platform);await Promise.all([fe(s),be(o),v(p.join(o,"index.html"),`<!DOCTYPE html>
<html lang="en">
  <head>
    <meta charset="UTF-8" />
  ...
  <body>
    <div id="root"></div>
    <script type="module" src="/src/main.${d.main}"></script>
  </body>
</html>
`),je(o,d.main,c,i.styling,f),i.styling==="CSS Modules"?v(p.join(o,"src/app/page.module.css"),Te()):Promise.resolve(),v(p.join(o,"src/app/globals.css"),Oe(i.styling)),De(o,d.api),v(p.join(o,".gitignore"),ze(d.api)),v(p.join(o,"package.json"),JSON.stringify(b,null,2)),he(o,c,d.config,i.styling,f),c?Fe(o):Promise.resolve(),_e(o)]),t.success(`Generated ${a.green}${c?"TypeScript":"JavaScript"}${a.reset} project with ${a.green}${i.styling}${a.reset} styling`);let{pm:l,failed:u}=ce(n.packageManager);u||t.info(n.packageManager?`Package manager: ${a.cyan}${l}${a.reset} (forced via --${l})`:`Package manager: ${a.cyan}${l}${a.reset}`),f&&g&&await Ce(o,g,r,l,e,i.appName,n.sign);let X=false;n.install===true?X=true:n.install===false?X=false:A()&&(X=await $({message:"Install dependencies now?",default:true}));let k=false;u||(k=await pe(o,l,X)),await v(p.join(o,"README.md"),Me(e,c,d,l,f,g)),t.plain(`
...
`),t.success("Get started:"),t.plain(`
  ${a.green}cd ${e}${a.reset}`),!k&&!u&&t.plain(`  ${a.green}${l} install${a.reset}`),f&&g?g==="android"?(t.plain(`
${a.bold}${a.cyan}Android Commands:${a.reset}`),t.plain(`  ${a.green}${m(l,"android")}${a.reset}          # Run on Android emulator/device`),t.plain(`  ${a.green}${m(l,"android:build")}${a.reset}    # Build APK`)):g==="ios"?(t.plain(`  ${a.green}${m(l,"ios")}${a.reset}  # Run on iOS (macOS only)`),t.plain(`  ${a.green}${m(l,"ios:build")}${a.reset}  # Build iOS app`)):t.plain(`  ${a.green}${m(l,"tauri:dev")}${a.reset}  # Launches ${g} desktop app`):t.plain(`  ${a.green}${m(l,"dev")}${a.reset}`),t.plain(`
${a.green}Happy coding!${a.reset}`);}async function We(){ae();let{projectName:e,flags:i}=oe(),n=q();t.plain(ie),t.info(`Detected OS: ${n}`);let r=e;r||(A()||(t.error("Project name required in non-interactive mode."),w(1)),r=await input({message:"Project name?",default:"my-bini-app",validate:s=>s?W(s)?true:"Lowercase letters, numbers, hyphens only. Max 50 chars.":"Name required."})),W(r)||(t.error("Invalid project name. Use lowercase letters, numbers, and hyphens only. Max 50 chars."),w(1));let o=await ge(i,r);await Le(r,o,i,n);}We().catch(e=>{t.error(`Fatal: ${e instanceof Error?e.message:String(e)}`),w(1);});