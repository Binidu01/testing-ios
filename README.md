...
- **Zero-dependency production server** via `bini-server` (web target) — pure Node.js `http`, serves `dist/` with ETag/304 support and proxies `/api/*`
- **Static SPA export** via `bini-export` (web target) — pre-renders every static route, generates a smart `404.html`, and strips platform files for GitHub Pages, Netlify static, S3, and more
...
| `npm run export` | Static SPA export — pre-renders routes, generates `404.html`, strips platform files (web target only) |
| `npm start` | Serve the production build via `bini-server` (web target only) |
...
> `export` and `start` only exist on web-target projects — `bini-export` and `bini-server` aren't installed for desktop/mobile scaffolds, since those ship as a native binary instead.

> Desktop/mobile projects also get `tauri:dev`, `tauri:build`, `android`, `android:build`, `ios`, and `ios:build` — see [Platforms](#platforms) below for the exact commands and requirements for each target.
...
> Web target only — desktop/mobile scaffolds don't install `bini-server` and ship as a native binary instead.
...
> Web target only — desktop/mobile scaffolds don't install `bini-export` and ship as a native binary instead.
...
Bini.js is a genuinely cross-platform React framework. One codebase — same routes, same API handlers, same components — targets the browser and compiles to a real native binary on every major desktop and mobile platform, not a browser wrapped in a window.

```bash
npx create-bini-app@latest my-app                                    # web (default)
...
Scaffold with `--platform web` (the default), `--platform windows`, `--platform macos`, `--platform linux`, `--platform android`, or `--platform ios` — or pick a platform interactively:
...
- **Shared API layer** — requests to `/api/*` route into the same Hono handlers on every target. On the web they're served by `bini-server` or an edge function; in packaged desktop/mobile apps they run in-memory against Hono's `fetch()` entrypoint. No separate mobile backend.
...
- **Force your package manager** — `--npm`, `--pnpm`, `--yarn`, or `--bun` pins the tool used for every command, so CI stays deterministic.

Each scaffolded project also gets its own README section with the exact commands and prerequisites for the platform you chose. The breakdown below is what you get for each target — web included, on equal footing with desktop and mobile.

### Web

**Features**
- File-based routing, nested layouts, per-route metadata, and automatic code splitting via `bini-router`.
- API routes via Hono — plain function handlers or full Hono apps in `src/app/api/`, served by the dev middleware, `bini-server` in production, or an edge function on deploy.
- Zero-dependency production server (`bini-server`) with ETag/304 support, graceful shutdown, and automatic port fallback.
- Static SPA export (`bini-export`) — pre-renders every static route and generates a smart `404.html` for any static host.
- Deploy anywhere — Netlify Edge Functions, Vercel Edge, Cloudflare Workers, Node.js, Deno.
- Dev overlay with Shiki-highlighted error panels, auto-imports, and GitHub Codespaces support out of the box.
...
| `npm run export` | Static SPA export — pre-renders routes, generates `404.html` |
| `npm run preview` | Preview the production build via Vite |

> `export` and `start` only exist on web-target projects — `bini-export` and `bini-server` aren't installed for desktop/mobile scaffolds, since those ship as a native binary instead. Conversely, desktop/mobile projects also get `tauri:dev`, `tauri:build`, `android`, `android:build`, `ios`, and `ios:build`, which don't exist on web-target projects — see the platform sections below for what each one does.

**Requirements**
- Node.js `>= 20.19.0` — that's it. No native toolchains, SDKs, or signing setup needed.
...
> iOS projects also get a bare `tauri` script (`"tauri": "tauri"`) in `package.json`, not just the wrappers above. Xcode's generated Run Script build phase shells out to it directly (`npm run tauri -- ios xcode-script ...`), so builds triggered from within Xcode fail with `Missing script: "tauri"` if it's absent — even though `npm run ios` works fine from the terminal. This script is only added for the iOS target; other platforms don't need it.
...
| [`bini-server`](https://npmjs.com/package/bini-server) | `latest` | Zero-dependency production server — ETag, timeouts, graceful shutdown (web target only) |
| [`bini-native`](https://www.npmjs.com/package/bini-native) | `latest` | Automatic native plugin wiring for desktop and mobile builds |
| [`@tauri-apps/cli`](https://v2.tauri.app) | `latest` | Native runtime — compiles the app to a real Windows/macOS/Linux binary or Android/iOS package |
...
| [`bini-export`](https://npmjs.com/package/bini-export) | `latest` | Static SPA export — per-route pre-rendering, smart 404.html (web target only) |
